...
struct XML_DATA* xml_load( const char* filename )
{
	...
	if( fp == NULL ){
        sprintf(printinfo, "file could not open: %s %s",__FUNCTION__,filename );
        seterr(printinfo);
		return NULL;
	}
	//get file size
		...
		case TagEnd:	//</tag>
			if( *p == '>' ){
				if( *v == '/' && strncmp( v+1, cur->name, MIN(XML_NODE_NAME_LEN-1, (int)(p-v-1)) )==0 ){
                    ...
                    DBG("## incorrect end tag for <%s>\n", cur->name );
				}
//				DBG("node name:%s  value:%s\n", cur->name, cur->value );
			}
			break;
