...
int loop_create( loop_list_t* l, int size, loop_delete_func del )
{
	...
	assert( l->items != NULL );
	if( l->items == NULL ){
		perror("failed when malloc items for loop");
		return -1;
	}
	return 0;
}
...
void* loop_search( loop_list_t* l, void* v, loop_search_func search )
{
	int i;
	pthread_mutex_lock( &l->mutex );
	for( i=l->head; i!=l->tail; i=(i+1)%l->size )
...
{
	return( (l->size+l->head-1)%l->size == l->tail );
}
